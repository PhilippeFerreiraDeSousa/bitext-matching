...
bool SentenceHandle::readNextSentence(SentPair& sent){
        ...
        // inserting line into word_buffer and updating the occurance number
        ...
        //if the loading is ok, adding the indexes for the mapping in the target language vector
        if(getline(*p_inputFile, line)){
                istringstream word_buffer(line);
                WordIndex w;
                ...
                fail = true;
        //loading indexes in the foreign language vector
        if(getline(*p_inputFile, line)){
                istringstream word_buffer(line);
                WordIndex w;
